...
MVP для построения категориального базиса драйверов сложности ML/AI-проектов и разложения текстовых кейсов по этому базису. Драйверы описывают объём и характер работ, но не стоимость проекта.

## Состав

1. `notebooks/01_build_driver_catalog.ipynb` — анализирует `data/train/*.md`, сопоставляет кейсы с каталогом и дополняет его.
2. `notebooks/02_infer_case_drivers.ipynb` — анализирует `data/test/*.md`, формирует распределения по категориям и requests to add.
